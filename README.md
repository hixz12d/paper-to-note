...
- **可直接入库的输出格式**
  最终输出是适合 `Obsidian` 使用的 `Markdown` 文件，支持 `[[#^ref-n|[n]]]` 引用跳转和 `LaTeX` 公式渲染（`$Zn^{2+}$`、`$K_d$` 等），便于继续整理、链接和检索。
...
- 笔记生成严格遵循模板和 prompt 规范
- 正文默认中文输出，仅保留化合物名、缩写等英文
- 支持 `Obsidian` 引用跳转（`[[#^ref-n|[n]]]` → 文末参考文献 `^ref-n`）
- 数学/化学公式使用 `LaTeX` 语法
- 表格渲染兼容性修复（前后空行）
- 文件名规范化（`{期刊缩写}--{年份}--{中文概括}.md`）
- 新增实验型论文输出示例
